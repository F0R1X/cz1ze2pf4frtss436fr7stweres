...
{
	//int ret = zjh.zjh_rc->command("hset chenzhoutables %s %s", tid.c_str(), data_str.c_str());
	int ret = zjh.zjh_rc->command("hset %s %s %s", zjh.conf["redis-db"]["table"].asString().c_str(), tid.c_str(), data_str.c_str());
	if (ret < 0) {
...
{
	//int ret = zjh.zjh_rc->command("hgetall chenzhoutables");
	int ret = zjh.zjh_rc->command("hgetall %s", zjh.conf["redis-db"]["table"].asString().c_str());
	if (ret < 0) {
...
{
	//int ret = zjh.zjh_rc->command("hdel chenzhoutables %s ", tid.c_str());
	int ret = zjh.zjh_rc->command("hdel %s %s ", zjh.conf["redis-db"]["table"].asString().c_str(), tid.c_str());
	if (ret < 0) {
